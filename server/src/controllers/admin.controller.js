...
export const createUser = async (req, res) => {
    try {
        ...
        if (!email || !password || !name || !tenant) {
            return res.status(400).json({
                error: 'Missing required fields: email, password, name, and tenant are required'
            });
        }
...
export const updateUser = async (req, res) => {
    try {
        const { id } = req.params;
        const { email, password, name, role, status, loginAttempts, isVerified, tenant } = req.body;
...
export const toggleRestrict = async (req, res) => {
    try {
        ...
        const action = updatedUser.status === 'RESTRICTED' ? 'restricted' : 'activated';
