...
const SummaryChart = React.lazy(
  () => import("@/AppComponents/Charts/SummaryChart")
);
const LogsAlertsTrendChart = React.lazy(
  () => import("@/AppComponents/Charts/LogsAlertsChart")
);
const TopIPSourcesTable = React.lazy(
  () => import("@/AppComponents/Table/TopIPSourcesTable")
);
const AlertsTable = React.lazy(
  () => import("@/AppComponents/Table/AlertsTable")
);
...
function useDebouncedCallback<T extends (...args: any[]) => any>(
  ...
  const ref = React.useRef<{ timeout?: number | NodeJS.Timeout; fn: T }>({
    fn,
  });
  useEffect(() => {
    ref.current.fn = fn;
...
const Dashboard: React.FC = () => {
  ...
  const handleDeleteAlerts = useCallback(
    async (id: string) => await deleteAlert(id),
    []
  );
  const handleDeleteLogs = useCallback(
    async (id: string) => await deleteLog(id),
    []
  );
  const handleResolveAlerts = useCallback(
    async (id: string) => await resolveAlert(id),
    []
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 px-6">
      <div className="max-w-full mx-auto">
        <Suspense fallback={<LoadingScreen />}>
          ...
          {/* Logs Table Section */}
          <Card className="px-6 mt-6 mb-6">
            <CardHeader className="">
              <div>
                <h1 className="text-xl font-bold">Log Details</h1>
                <CardDescription>
                  Details of all the creatd logs
                </CardDescription>
              </div>
            </CardHeader>
